...
data = np.genfromtxt('R_RD'+'.csv', delimiter=',')
data_strain = data[:,0]
r_val = data[:,1]
...
def swiftVoce(x , C11, C12, C13, C21, C22, C23):
    return np.piecewise(x, [x<0.003],[lambda x:C11 + C12*(1-eur**(-C13*x)), lambda x:C21 + C22*(1-eur**(-C23*x))])
...
popt, pcov= curve_fit(swiftVoce, X, Y, maxfev= 100000000 , bounds=((0, 0, 0, 0, 0, 0), (np.inf, np.inf, np.inf, np.inf, np.inf, np.inf)))
...
y_line1_extend_single = swiftVoceT(x_line_extend_single, c1, c2, c3)
...
plt.legend(['Raw Data', 'Piecewise function'])
plt.xlabel("True Strain")
plt.ylabel("True Stress")
plt.title("Optimised Swift Voce curve extended to large strain for R-Value RD")
plt.savefig('Fitted Swift Voce curve extended to large strain (R value RD)')
print(np.max(y_line_extend_piecewise_second))
print(np.max(y_line1_extend_single))

plt.figure(2)
plt.scatter(X,Y)
x_line_maxStress1 = np.arange(0, 0.004, 0.001)
x_line_maxStress2 = np.arange(0.003, 0.162, 0.001)
y_line_maxStress_piecewise1 = swiftVoce(x_line_maxStress1, c11, c12, c13, c21, c22, c23)
y_line_maxStress_piecewise2 = swiftVoce(x_line_maxStress2, c11, c12, c13, c21, c22, c23)
#y_line_maxStress_single = swiftVoceT(x_line_maxStress, c1, c2, c3)
plt.plot(x_line_maxStress1, y_line_maxStress_piecewise1, color = 'yellow')
plt.plot(x_line_maxStress2, y_line_maxStress_piecewise2, color = 'red')
#plt.plot(x_line_maxStress, y_line_maxStress_single, color = 'black')
plt.legend(['Raw Data',  'Piecewise function (small strain)', 'Piecewise function (large strain)'])
plt.xlabel("True Strain")
plt.ylabel("True Stress")
plt.title("Optimised Swift Voce curve for R-Value RD")
plt.savefig('Fitted Swift Voce curve (R value RD)')
