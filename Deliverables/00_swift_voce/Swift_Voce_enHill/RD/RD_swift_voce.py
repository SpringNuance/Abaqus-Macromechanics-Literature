...
data = np.genfromtxt("RD"+'.csv', delimiter=',')
data_strain = data[:,0]
data_stress = data[:,1]
...
plt.savefig('Fitted Swift Voce curve extended to large strain (RD)')
...
plt.plot(x_line_maxStress_small, y_line_maxStress_piecewise_small, color = 'yellow')
...
plt.xlabel("True Strain")
plt.ylabel("True Stress")
plt.title("Optimised Swift Voce for RD")
plt.savefig('Fitted Swift Voce curve(RD)')
