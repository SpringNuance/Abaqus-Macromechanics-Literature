...
data = np.genfromtxt('R_TD'+'.csv', delimiter=',')
data_strain = data[:,0]
r_val = data[:,1]
...
popt, pcov= curve_fit(swiftVoce, X[93:], Y[93:], maxfev= 100000000 , bounds=((0, 0, 0, 0, 0, 0), (np.inf, np.inf, np.inf, np.inf, np.inf, np.inf)))
...
plt.title("Optimised Swift Voce curve extended to large strain for R-Value TD")
plt.savefig('Fitted Swift Voce curve extended to large strain (R value TD)')
print(np.max(y_line_extend_piecewise_second))
print(np.max(y_line_extend_single))

plt.figure(2)
plt.scatter(X,Y)
x_line_maxStress = np.arange(0.008, 0.166, 0.001)
...
#plt.plot(x_line_maxStress, y_line_maxStress_single, c1, c2, c3, color = 'black')
plt.legend(['Raw Data',  'Piecewise function'])
plt.xlabel("True Strain")
plt.ylabel("True Stress")
plt.title("Optimised Swift Voce curve for R-Value TD")
plt.savefig('Fitted Swift Voce curve (R value TD)')
